...
5. Right click on teamcode, then click open module settings. Set the source compatibility to Java 1.8. As of HAL 1.1.0, HAL will only support Java 8, as all legal FTC control devices support Java 8 as of the 2020-2021 season.

6. Go to your teamcode's build.release.gradle file and add this line to the end: 

```gradle 
implementation 'io.github.schs-robotics:hal9001:x.x.x' 
```
...
7. Plug your robot controller phone into the computer and swipe down, then click where it says "USB for charging" and change it to File Transfers. (If you are using the rev control hub, just plug it in to your computer and this should happen automatically)

8. Download the latest .so file from the [HAL9001-OpenCV-Repackaged repository](https://github.com/SCHS-Robotics/HAL9001-OpenCV-Repackaged) (check the releases section of that repository).

9. Open file explorer (or finder for mac people) and navigate to the phone (or control hub). It should show up just like a USB drive.

10. Copy paste the .so file into the FIRST folder on the phone's internal storage.

11. Celebrate, you're done!
